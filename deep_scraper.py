...
#
if len(sys.argv) != 2:
    error('no course string specified', 1)

course_str = sys.argv[1]        # e.g. 'cascs111'
...
for table in tables:
    ...
    for row in rows:
        ...
        if rowspan == 1:
            ...
            if len(tds) == 0:
                error('section list row has no data cells', 3)

            section_id = row.attrib['data-section']
            if section_id is None:
                error('section list row has no data-section attribute', 4)
            ...
            if len(tds) != len(ROW_KEYS):
                error('unexpected number of data cells in section list row '
                      '(expected {}, found {})'.format(len(ROW_KEYS), len(tds)), 5)
...
if len(seats) == 0:
    error('could not get open seats numbers', 6)
